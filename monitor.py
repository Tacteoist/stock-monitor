...
portfolio = [
    "ABBV","ABM","ABR","AGNC","ALB","AMCR","ANDE","AVNT","AWR","BEN"  # ... continue full list
]
...
def load_dividend_cache():
    ...
    for ticker in portfolio:
        ...
        if old_data.get(ticker,0) > 0 and annual_div==0:
...
# ---------------- INTRADAY CACHE ---------------- #
cache_file = "data/intraday_cache.json"
    ...
    for ticker in tickers:
        try:
            ...
            intraday_cache[ticker] = {
                "last_price": float(data[ticker].iloc[-1]),
                "last_timestamp": data[ticker].index[-1].to_pydatetime().isoformat()
            }
            time.sleep(0.5)
        except:
            continue
...
market_alert=""
if panic_ratio>=0.05: market_alert="🔥 MARKET PANIC MODE"
elif crash_ratio>=0.15: market_alert="⚠️ MARKET SELL-OFF"
